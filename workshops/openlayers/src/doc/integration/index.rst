...
OpenLayers provides the mapping foundation for your web application. To build an application with rich user interface components, OpenLayers is best paired with a UI framework. There are a number of full-featured UI frameworks available; `Ext JS`_, Dojo_ (via Dijit), and YUI_ are popular frameworks with good layout controls and widgets. This module provides examples of OpenLayers integration with jQuery_ and `jQuery UI`_ as well as with `Ext JS`_.

In this module you will use the jQuery UI and Ext JS libraries to create widgets that interact with your OpenLayers map.

.. _Ext JS: https://www.sencha.com/products/extjs/
